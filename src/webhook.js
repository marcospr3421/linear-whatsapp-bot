...
const verifySignature = (signature, rawBody) => {
    ...
    if (!signature || typeof signature !== 'string')
        return false;
    const computed = crypto_1.default.createHmac('sha256', WEBHOOK_SECRET).update(rawBody).digest('hex');
    try {
...
const startWebhookServer = (whatsappClient) => {
    app.get('/health', (_req, res) => {
        res.json({ status: 'ok', service: 'ada-linear-bot', timestamp: Date.now() });
    });
    app.post('/linear-webhook', express_1.default.json({
        verify: (req, _res, buf) => {
            req.rawBody = buf;
        },
    }), async (req, res) => {
        try {
            ...
            if (type === 'Issue') {
                ...
                message = `${messages_1.ADA}: 💬 *Novo Comentário em ${issueIdentifier}*\n\n👤 *${author}* comentou:\n"${body}"\n\n🔗 ${url || data.url}\n\n💡 _Deseja responder a este comentário, meu bem? É só digitar sua resposta abaixo! 🥰💖_`;
                if (NOTIFY_NUMBER && data.issue?.identifier) {
                    if (!sessions_1.sessions[NOTIFY_NUMBER]) {
                        sessions_1.sessions[NOTIFY_NUMBER] = { history: [] };
                    }
                    sessions_1.sessions[NOTIFY_NUMBER].pendingReplyIssueId = data.issue.identifier;
                    sessions_1.sessions[NOTIFY_NUMBER].pendingReplyIssueTitle = issueTitle;
                }
            }
            else if (type === 'Project' && action === 'create') {
                message = `${messages_1.ADA}: 🆕 📂 *Novo Projeto*\n\n📛 *${data.name}*\n📊 Status: ${data.state}\n\n🔗 ${url || data.url}`;
            }
            if (message) {
