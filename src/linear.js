...
exports.getTeamWeeklyActivity = exports.getMyDailyFocus = exports.generateWeeklyReport = exports.searchLinearIssuesAdvanced = exports.getProjectSummary = exports.updateIssueDueDate = exports.addIssueComment = exports.findSimilarIssues = exports.listTeamMembers = exports.updateIssuePriority = exports.assignIssue = exports.closeAllProjectIssues = exports.updateProjectStatus = exports.updateIssueStatus = exports.searchLinearIssues = exports.archiveLinearProject = exports.listLinearProjects = exports.createLinearProject = exports.createLinearIssue = void 0;
...
const createLinearIssue = async (title, description, priority, projectIdOrName) => {
    try {
        ...
        const teamId = process.env.LINEAR_TEAM_ID || (await linearClient.teams()).nodes[0].id;
...
const createLinearProject = async (name, description) => {
    try {
...
exports.createLinearProject = createLinearProject;
const listLinearProjects = async () => {
    try {
        const projects = await linearClient.projects({
...
exports.listLinearProjects = listLinearProjects;
const archiveLinearProject = async (projectIdOrName) => {
    try {
        let projectId = projectIdOrName;
...
exports.archiveLinearProject = archiveLinearProject;
const searchLinearIssues = async (query) => {
    try {
        ...
        const results = await Promise.all(issues.nodes.map(async (i) => {
...
const updateIssueStatus = async (identifierOrTitle, statusName) => {
    try {
        // We first need to find the state ID for the given status name
        const workflowStates = await linearClient.workflowStates();
        const targetState = workflowStates.nodes.find(s => s.name.toLowerCase().includes(statusName.toLowerCase()));
        if (!targetState)
...
const updateProjectStatus = async (projectNameOrId, statusName) => {
    try {
        ...
        const targetProjectStatus = projectStatuses.nodes.find((s) => s.name.toLowerCase() === state.toLowerCase() ||
...
const closeAllProjectIssues = async (projectNameOrId) => {
    try {
        let projectId = projectNameOrId;
        if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(projectNameOrId)) {
            const projects = await linearClient.projects({
        ...
        const workflowStates = await linearClient.workflowStates();
        const doneState = workflowStates.nodes.find(s => s.name.toLowerCase() === 'done' ||
            s.name.toLowerCase() === 'concluído' ||
            s.name.toLowerCase() === 'concluido');
        if (!doneState)
            return { success: false, error: 'Estado de conclusão não encontrado' };
...
exports.closeAllProjectIssues = closeAllProjectIssues;
const assignIssue = async (identifier, assigneeName) => {
    try {
        const users = await linearClient.users();
        const targetUser = users.nodes.find(u => u.name.toLowerCase().includes(assigneeName.toLowerCase()) ||
...
const updateIssuePriority = async (identifier, priority) => {
    try {
        const issue = await findIssueByIdentifier(identifier);
        if (!issue)
            return { success: false, error: 'Tarefa não encontrada' };
...
exports.updateIssuePriority = updateIssuePriority;
...
const findSimilarIssues = async (title) => {
    try {
...
const addIssueComment = async (identifier, body) => {
    try {
        const issue = await findIssueByIdentifier(identifier);
        if (!issue)
            return { success: false, error: 'Tarefa não encontrada' };
...
const updateIssueDueDate = async (identifier, dueDate) => {
    try {
        const issue = await findIssueByIdentifier(identifier);
        if (!issue)
            return { success: false, error: 'Tarefa não encontrada' };
...
exports.updateIssueDueDate = updateIssueDueDate;
const getProjectSummary = async (projectName) => {
    try {
        const projects = await linearClient.projects({
        ...
        const byStatus = {};
        const byAssignee = {};
        const issuesWithState = await Promise.all(issues.nodes.map(async (issue) => {
...
const searchLinearIssuesAdvanced = async (query, assigneeName) => {
    try {
        const filter = {
        ...
        if (assigneeName) {
            const users = await linearClient.users();
            const user = users.nodes.find((u) => u.name.toLowerCase().includes(assigneeName.toLowerCase()) ||
                u.displayName.toLowerCase().includes(assigneeName.toLowerCase()));
            if (user) {
                filter.assignee = { id: { eq: user.id } };
            }
        }
        const issues = await linearClient.issues({ filter: filter, first: 10 });
        const results = await Promise.all(issues.nodes.map(async (i) => {
            const state = i.state ? await i.state : undefined;
...
exports.searchLinearIssuesAdvanced = searchLinearIssuesAdvanced;
const generateWeeklyReport = async () => {
    try {
        ...
        const issuesWithStateName = await Promise.all(issues.nodes.map(async (i) => {
            const state = i.state ? await i.state : undefined;
            return {
                node: i,
...
exports.getMyDailyFocus = getMyDailyFocus;
const getTeamWeeklyActivity = async () => {
    try {
        const weekAgo = new Date();
        weekAgo.setDate(weekAgo.getDate() - 7);
        const issues = await linearClient.issues({
        ...
        const activityList = await Promise.all(issues.nodes.map(async (i) => {
            const state = i.state ? await i.state : undefined;
            const assignee = i.assignee ? await i.assignee : undefined;
            return {
                identifier: i.identifier,
...
exports.getTeamWeeklyActivity = getTeamWeeklyActivity;
